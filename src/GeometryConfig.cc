...
G4LogicalVolume *ProcessRotation(const string &name, YAML::Node node)
{
  ...
  G4Material* material = child->GetMaterial();
  if (node["material"]) {
    string mat_name = node["material"].as<string>();
    material = G4Material::GetMaterial(mat_name);
  }

  auto logical = CreateBoxVolume(name, fabs(v.x()), fabs(v.y()), fabs(v.z()), material);
